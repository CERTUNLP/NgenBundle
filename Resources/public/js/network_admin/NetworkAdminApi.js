...
var NetworkAdminApi = ApiClient.extend({
    config: function () {
//        this.api.add("networks", {stripTrailingSlash: true});
//        this.api.networks.add("report", {stripTrailingSlash: true});
        $.subscribe('/cert_unlp/network/admin/activate', $.proxy(this.activate, this));
        $.subscribe('/cert_unlp/network/admin/desactivate', $.proxy(this.desactivate, this));
        $.subscribe('/cert_unlp/network/admin/new', $.proxy(this.create, this));
        $.subscribe('/cert_unlp/network/admin/update', $.proxy(this.update, this));
    },
    addDefaultChannel: function () {
        this.api.add("admins", {stripTrailingSlash: true, url: 'networks/admins'});
        this.defaultChannel = this.api.admins;
    },
