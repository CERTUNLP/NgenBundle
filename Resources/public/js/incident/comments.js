...
(function (window, $, easyXDM) {
    ...
    var FOS_COMMENT = {
        ...
        post: function (url, data, success, error, complete) {
            ...
            $.post(url + (url.split('?').length > 1 ? '&' : '?' )+ 'apikey=' + window.api_key, data, success).fail(wrappedErrorCallback).always(wrappedCompleteCallback);
        },
        ...
        get: function (url, data, success, error) {
            ...
            $.get(url + (url.split('?').length > 1 ? '&' : '?') + 'apikey=' + window.api_key, data, success).fail(wrappedErrorCallback);
        },
