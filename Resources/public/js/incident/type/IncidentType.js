...
var IncidentType = Frontend.extend({
    ...
    doChangeState: function (event) {
        if (this.eventTarget.data('action') == 'reactivate') {
            $.publish('/cert_unlp/incident/type/activate', [this.eventTarget.parents('tr').data('id'), $.proxy(this.stateChanged, this)]);
        } else {
            if (this.eventTarget.data('action') == 'desactivate') {
                $.publish('/cert_unlp/incident/type/desactivate', [this.eventTarget.parents('tr').data('id'), $.proxy(this.stateChanged, this)]);
            }
        }
